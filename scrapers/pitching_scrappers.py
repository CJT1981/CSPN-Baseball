...
for year in years:
    for team in teams:
        ...
        team_tables = team_html.find('table', id='players_standard_pitching')

        # If the table is not found, skip to the next iteration
        if team_tables is None:
            print(f"No pitching data found for {url_team} in {year}. Skipping...")
            continue
...
master_dataframe.to_csv(r'C:\Users\chris\CSPN-Baseball\data\y00-26_pitching_data.csv', index=False)   
