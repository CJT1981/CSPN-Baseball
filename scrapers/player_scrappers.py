...
player = []
position, bats, throws, height, weight, debut, last_game = None
...
for letter in string.ascii_lowercase:
    ...
    for player in player_rows:
        ...
        html_profilepage = requests.get(profile_url)

        # Adding a delay between requests to avoid overwhelming the server
        time.sleep(3)

        # Check if the request was successful
        if html_profilepage.status_code != 200:
            print(f"Failed to retrieve profile data for {player_name}. Status code: {html_profilepage.status_code}")
            continue
